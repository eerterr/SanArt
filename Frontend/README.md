...
бэкенду (`../Backend`) через `fetch`. Три страницы: `login.html`, `index.html`
(дашборд), `events.html` (ивенты).
...
```
Frontend/
  login.html / index.html / events.html
  assets/
    ...
    css/
      ...
      pages/                  — точечные правки под конкретную страницу
    js/
      ...
      components/
        ...
        ai-widget.js              — плавающий чат (пока без реального ИИ-ответа)
      pages/
...
- Все данные — с бэкенда: `GET /api/institutions`, `/api/dashboard?institution_id=`,
  `/api/events?institution_id=`, `/api/events/<id>`. Никаких моков в JS больше нет.
